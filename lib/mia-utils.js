...
export function summarizeMessage(message) {
  try {
    switch (message?.role) {
      ...
      case 'error':
        return `Error: ${message.content || 'Unknown error'}`;
...
export class MiaTransformStream extends Transform {
  ...
  _extractMessage(chunk) {
    try {
      ...
      try {
        ...
        this.logger.error(
          { err, responseChunk: responseChunk.substring(0, 200) + '...' },
          'Error extracting message from chunk'
        );
        // Return the error to the client
        return {
          role: 'error',
          content: `Error processing response: ${err.message}`,
        };
      }
    } catch (outerErr) {
  ...
  _transform(chunk, encoding, callback) {
    try {
      ...
      try {
        summarizedContent = summarizeMessage(message, this.logger);
      } catch (err) {
        this.logger.error(
          {
            err,
        ...
        summarizedContent = 'Error processing tool response';
      }
